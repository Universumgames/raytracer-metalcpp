 ...
 */
sfColor Raytracer_trace_ray(const Raytracer *raytracer, const Ray ray, const int depth) {
    if (depth > raytracer->max_depth) {
        return sfBlack; // Return black for rays that reach max recursion depth
    }
    ...
    const sfColor light_color = raytracer->scene.lights[0].color;
    ...
    switch (nearest_objecttype) {
        ...
        case NONE:
        default: {
            return sfBlack; // Return background color if no object is hit
        }
    }
...
void Raytracer_render(const Raytracer *raytracer, Bitmap bitmap) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            const sfColor pixel_color = Raytracer_trace_ray(raytracer, ray, raytracer->max_depth);

            // Set the pixel color in the bitmap
            Bitmap_put_pixel(bitmap, x, height-y, pixel_color);
        }
    }
}